...
class IOUfitModule(nn.Module):
    def __init__(self, in_features, hidden_features=None):
        ...
        self.conv2 = Conv(in_channels=1, out_channels=1, kernel_size=3, stride=1, padding=1) #512
        self.conv3 = Conv(in_channels=1, out_channels=1, kernel_size=3, stride=1, padding=1) #512
        self.conv4 = Conv(in_channels=1, out_channels=1, kernel_size=3, stride=1, padding=1) #512
    ...
    def forward(self, pred, gt):
        ...
        out = self.conv2(out) #N,1,512
        out = self.conv3(out) #N,1,512
        out = self.conv4(out) #N,1,512
        # out = self.conv5(out) #N,1,32
        # out = self.conv6(out) #N,1,16
        out = torch.squeeze(out, dim=1)  #N,512
        out = self.fc5(out) #N,1
        return out
