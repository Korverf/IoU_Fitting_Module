...
class IOUfitModule(nn.Module):
    def __init__(self, in_features, hidden_features=None):
        ...
        self.fc1 = FC(in_channels=in_features, feedforward_channels=2048, out_channels=2048,
        ...
        self.conv2 = Conv(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1) #1024
        self.conv3 = Conv(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1) #512
        self.conv4 = Conv(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1) #256
        self.conv5 = Conv(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1) #128
        self.conv6 = Conv(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1) #64
        self.conv7 = Conv(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1)  # 32
        self.conv8 = Conv(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1)  # 16
        self.fc9 = FC(in_channels=16, feedforward_channels=16, out_channels=1,
                      add_residual=False, with_act=False, with_bn=False)
    ...
    def forward(self, pred, gt):
        ...
        out = self.conv7(out)  # N,1,16
        out = self.conv8(out)  # N,1,16
        out = torch.squeeze(out, dim=1)  #N,16
        out = self.fc9(out) #N,1
        return out
