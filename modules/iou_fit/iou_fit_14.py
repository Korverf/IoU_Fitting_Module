...
class IOUfitModule(nn.Module):
    def __init__(self, in_features, hidden_features=None):
        ...
        #               act_cfg=dict(type='ReLU'), add_residual=False)
        self.fc1 = FC(in_channels=in_features, feedforward_channels=1024, out_channels=1,
                      act_cfg=dict(type='Sigmoid'), add_residual=False, with_act=False, with_bn=True)
...
class FC(nn.Module):
    def __init__(self,
        ...
        layers.append(Linear(in_channels, feedforward_channels))
        if with_bn == True:
            layers.append(nn.BatchNorm1d(feedforward_channels))
        layers.append(nn.Sequential(self.relu, 
