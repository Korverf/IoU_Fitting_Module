...
class IOUfitModule(nn.Module):
    def __init__(self, in_features, hidden_features=None):
        ...
        #               act_cfg=dict(type='ReLU'), add_residual=False)
        self.fc5 = FC(in_channels=in_features, feedforward_channels=hidden_features, out_channels=1,
                      act_cfg=dict(type='GELU'), add_residual=False, with_bn=False)
...
class FC(nn.Module):
    def __init__(self,
        ...
        layers.append(Linear(feedforward_channels, out_channels))
        if with_bn == True:
