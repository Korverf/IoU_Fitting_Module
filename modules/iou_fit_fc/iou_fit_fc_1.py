...
class IOUfitModule(nn.Module):
    def __init__(self, in_features, hidden_features=None):
        ...
        self.fc2 = FC(in_channels=512, out_channels=256, act_func='ReLU', with_act=True) #256
        self.bottle2 = Bottleneck(in_channels=256)
        self.fc3 = FC(in_channels=256, out_channels=128, act_func='ReLU', with_act=True) #128
        self.bottle3 = Bottleneck(in_channels=128)
        self.fc4 = FC(in_channels=128, out_channels=64, act_func='ReLU', with_act=True) #64
        self.fc5 = FC(in_channels=64, out_channels=32, act_func='ReLU', with_act=True) #32
        self.fc6 = FC(in_channels=32, out_channels=16, act_func='ReLU', with_act=True) #16
        self.fc7 = FC(in_channels=16, out_channels=1, with_act=False)
    ...
    def forward(self, pred, gt):
        ...
        out = self.bottle1(out) # N,512
        out = self.fc2(out) #N,256
        out = self.bottle2(out)  # N,256
        out = self.fc3(out) #N,128
        out = self.bottle3(out)  # N,128
        out = self.fc4(out) #N,64
        out = self.fc5(out) #N,32
        out = self.fc6(out) #N,16
