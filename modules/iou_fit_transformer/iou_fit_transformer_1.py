...
class IOUfitModule(nn.Module):
    def __init__(self, in_channels, inter_channels=None):
        ...
        self.out_fc = Linear(inter_channels, 1)
    ...
    def forward(self, pred, gt):
        ...
        out = self.attention1(query=input, key=input, value=input)
...
class IOUfitTransformer(nn.Module):
    ...
    def forward(self,
        ...
        value_emb = self.v_project(value)#N,512
        # if key_emb.dim()<3:
        #     print(key_emb.dim())
        similarity = torch.matmul(query_emb, key_emb.transpose(0,1)) / query_emb.size(-1)
        spft_similarity = F.softmax(similarity, dim=-1)  # B,N,N
        out = torch.bmm(spft_similarity, value_emb) / spft_similarity.size(-1)
        attn_out = self.out_project(out)
