...
class IOUfitModule(nn.Module):
    def __init__(self, in_channels, inter_channels=None):
        ...
        self.attention1 = IOUfitTransformer(in_channels=8, inter_channels=inter_channels)
        ...
        self.mse_linter_channelsoss = nn.MSELoss(reduction='sum')
        self.loss_weight = 10.0

    def forward(self, pred, gt):
        input = torch.stack([pred, gt], dim=1) #N,2,8
        out = self.attention1(query=input, key=input, value=input) #N,2,256
...
class IOUfitTransformer(nn.Module):
    ...
    def forward(self,
        ...
        query_emb = self.q_project(query)#N,2,256
        key_emb = self.k_project(key)#N,2,256
