...
class IOUfitModule(nn.Module):
    def __init__(self, in_channels, inter_channels=None):
        ...
        self.attention1 = IOUfitTransformer(in_channels=in_channels, inter_channels=inter_channels)
        #self.feedforward1 = FFN(2 * inter_channels, 2 * inter_channels, act_cfg=dict(type='ReLU', inplace=True))
        #self.out_fc = Linear(2 * inter_channels, 1)
    ...
    def forward(self, pred, gt):
        input = torch.cat([pred, gt], dim=-1) #N,8
        out = self.attention1(query=input, key=input) #N

        # N = out.size(0)
        # out = out.view(N, -1) #N,512
        # out = self.feedforward1(out) #N,512

        # out = self.out_fc(out) #N,1
...
class IOUfitTransformer(nn.Module):
    ...
    def __init__(self, in_channels, inter_channels):    #, bn_layer=False
        ...
        self.q_project = nn.Sequential(Linear(in_channels, inter_channels),nn.ReLU())
        #self.k_project = nn.Sequential(Linear(in_channels, inter_channels),nn.ReLU())
        #self.v_project = Linear(in_channels, inter_channels)
        # if bn_layer:
        #     self.out_bn = nn.BatchNorm1d(self.inter_channels)
        
        self.out_project1 = nn.Sequential(
            Linear(inter_channels, inter_channels),nn.ReLU())
        self.out_project2 = Linear(inter_channels, 1)
    ...
    def forward(self,
        ...
        query_emb = self.q_project(query)#N,512
        #key_emb = self.k_project(key)#N,512
        #out_embed = torch.cat([query_emb, key_emb], dim=-1)
        out = self.out_project1(query_emb)
        out = self.out_project2(out)
        #value_emb = self.v_project(value)#N,2,256
        # if key_emb.dim()<3:
        #     print(key_emb.dim())
        #similarity = torch.matmul(query_emb, key_emb.transpose(1,2)) / query_emb.size(-1) #N,2,2

        #similarity = torch.cosine_similarity(query_emb, key_emb, dim=1)
        #similarity = (similarity + 1) / 2
        #spft_similarity = F.softmax(similarity, dim=-1)  #N,2,2
        #out = torch.bmm(spft_similarity, value_emb) / spft_similarity.size(-1) #N,2,256
        #attn_out = self.out_project(out) #N,2,256
        #attn_out = self.out_bn(out_proj.transpose(1, 2)).transpose(1, 2)
        #out = (similarity[:, 0, 1] + similarity[:, 1, 0]) / 2
        return out #attn_out
