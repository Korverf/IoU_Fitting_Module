...
def parse_args():
    ...
    parser.add_argument('--method', type=str, default='gwd_loss')
...
def main(args):
    ...
    for j in range(1, args.val_num+1):
        ...
        with torch.no_grad():
            ...
            GWD_compute = GDLoss(loss_type='kld', representation='xy_wh_r', fun='log1p', tau=1.0, alpha=1.0)
            gwds = GWD_compute(rbboxes1_new, rbboxes2_new).clamp(min=1e-6, max=1)

            for i in range(len(IoU_targets)):
                dif = abs(IoU_targets[i] - gwds[i])
                mean_dif += dif
                if dif < 0.05:
                    total_score_5 += 1
                if dif < 0.01:
