...
def data_generator(N):
    ...
    count = 0
    while count < N:
        p1 = np.random.rand() * 4  #确定目标大小
        if 0 <= p1 < 1: #小
            xy1 = 2 * np.random.rand(1, 2) - 1  #(-1,1)
            wh1 = np.random.uniform(low=2e-3, high=0.02, size=(1, 2))
        elif 1 <= p1 < 2: #中
            xy1 =  2 * np.random.rand(1, 2) - 1  #(-1,1)
            wh1 = np.random.uniform(low=0.02, high=0.2, size=(1, 2))
        else: #大
        ...
        p2 = np.random.rand() * 4 #确定xy2与xy1的偏差
        if 0 <= p2 < 1:
            if p1 < 2 :
