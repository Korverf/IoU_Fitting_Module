...
from iou_fit_24 import get_model
...
def parse_args():
    ...
    parser.add_argument('--method', type=str, default='iou_fit_module_29')
    parser.add_argument('--iters', default=1000000, type=int)
    parser.add_argument('--print_inter', type=int, default=1000,
...
def main(args):
    ...
    for iter in range(1, num_iters+1):
        ...
        rbboxes1, rbboxes2 = data_generator(args.batch_size)    # B,N,5
        ...
        if iter % args.val_inter == 0:
            ...
            for j in range(1, args.val_num+1):
                rbboxes1, rbboxes2 = data_generator(1)
                iou_fit_value = iou_fit_module.forward(rbboxes1, rbboxes2)
                iou_fit_value = iou_fit_value[:, 0].clamp(min=1e-6)
                IoU_targets = obb_overlaps(rbboxes1, rbboxes2, is_aligned=True).squeeze(
                    1).clamp(min=1e-6)
                dif = abs(IoU_targets - iou_fit_value)
                if dif < 0.05:
                    total_score_5 += 1
                if dif < 0.01:
                    total_score_1 += 1
            ...
            total_score_5 = (total_score_5 / args.val_num) * 100
            total_score_1 = (total_score_1 / args.val_num) * 100
