...
def get_ratio_scale(category_id, annotations, ratio_interval, scale_interval,width_interval,height_interval):
    ratio_lists=[]
    scale_lists=[]
    width_list=[]
    height_list= []
    for item in annotations:
        ...
        width_list.append(item[2])
        height_list.append(item[3])

    fig1, axs = pyplot.subplots(3, figsize=(10,7))
    if category_id != None:
    ...
    # width
    x, y = generate_axes(width_list, width_interval, 500)
    y = [item / len(annotations) for item in y]
    ...
    # height
    x, y = generate_axes(height_list, height_interval, 500)
    y = [item / len(annotations) for item in y]

    axs[2].bar(x, y)
    axs[2].set_title('height(interval= %d )' % height_interval, fontsize=15)
    axs[2].yaxis.set_major_formatter(PercentFormatter(1))  # 纵轴显示百分数
    # pyplot.show()
    return
...
def compute_ratio_scale(annotations_list, ratio_interval, scale_interval, width_interval, height_interval):
    ...
    for cat_ in categories:
        ...
        #get_ratio_scale(cat_, annotations, ratio_interval, scale_interval, width_interval, height_interval)
    ...
    get_ratio_scale(None, annotations_all, ratio_interval, scale_interval, width_interval, height_interval)
...
if __name__=='__main__':
    #f=open('/home/yyw/yyf/Dataset/HRSC2016/Train/HRSC_L1_train_F1.json','r')
    ...
    compute_ratio_scale(annotations_list, ratio_interval, scale_interval, width_interval, height_interval)
